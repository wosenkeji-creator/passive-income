...
- 2026-09-01 | 补上了 `isWafChallenge` 的一个覆盖盲区：状态码+body 那条判定分支在集成层从未被测到，因为 mock 总是发 `x-amzn-waf-action`，而实现读到 header 就短路返回。突变验证：把 body 标记读取改成空串，改动前只有 1 个测试失败（waf.ts 单测），补测后两个独立突变体各失败 2 个测试。CloudFront 并没有义务附带那个 header，所以 fallback 必须自己有端到端覆盖（`e7fbc16`）。
- 2026-09-01 | 按次计费接上（`85c9503`）。此前 `grep -rn "charge" src/ .actor/` 零命中：Actor 抓完就 `pushData`，一分钱不收 —— 这是它唯一无法产生收入的原因，抓取侧其实早就通了。
- 2026-09-01 | 计费形状：一条结果一个 `job-result` 事件，**先扣费再交付**，不用 batch `count`。SDK 自己的文档说 batch 可能只被部分满足（`chargedCount < count`），那会留下"已经承诺但没买到"的工作量；逐条扣则每次停止决策都发生在价值离开 Actor 之前。
- 2026-09-01 | 计费里最容易写错的一处：`chargedCount === 0` 有两种完全相反的含义。`eventChargeLimitReached` 为 false 时是这个 Actor 根本不在 PPE 定价上（`ChargingManager.charge` 正是返回这个并只警告一次），结果仍然必须交付；为 true 时才是预算耗尽，此时交付就是白干。把两者混为一谈会让每一次免费运行静默返回空结果。已用一个专门的测试钉住。
- 2026-09-01 | SDK 在超预算时会**故意多扣一个事件**让平台终止运行。那一条已经收过钱，所以照常交付，之后才停 —— 不是丢掉它。
- 2026-09-01 | 本地验证走真实 `Actor.charge` 而不只是测试替身，分两个 mode，因为一次运行验不了两件事：本地 `calculateEventPrice` 对任何事件都返回名义 $1 用于判断预算，但按声明价计费，声明 $0.003 时两者差 300 倍，没有哪个预算能既触发停止又匹配真实价格。`declared` 用 `APIFY_ACTOR_PRICING_INFO` 注入 `.actor/pricing.json`，验证所扣事件名确实是已声明的计费事件；`budget` 不声明价格让两侧一致、预算可达，验证预算耗尽会停止交付。突变：把事件名改成未声明的，`check:ppe` FAIL 退出 1 —— 未声明的事件在线上按 $0 计，Actor 会白跑。
- 2026-09-01 | 未做且不代做：Apify Console 里应用 $0.003 定价、Store 上架。按 `CLAUDE.md` 的「支付/上架必须人工确认」，这两步留给人工。因此 G2 未过，代码侧的按次计费已完成。
